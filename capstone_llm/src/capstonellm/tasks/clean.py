...
def clean(spark: SparkSession, environment: str, tag: str):
    if environment == "localfs":
        ...
        s3_input_path = f"s3a://dataminded-academy-capstone-llm-data-us/input/{tag}/"
        s3_output_path = f"s3a://dataminded-academy-capstone-llm-data-us/cleaned/{tag}/"
    ...
    df.write.mode("overwrite").json(s3_output_path)

    count = df.count()
    df.repartition(count).write.mode("overwrite").json(s3_output_path)
